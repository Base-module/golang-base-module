...
func NewTestClien(fn RoundTripFunc) *http.Client {
	return &http.Client{
		Transport: fn,
	}
}
...
func TestTools_PushJSONToRemote(t *testing.T) {
	client := NewTestClien(func(req *http.Request) *http.Response {
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		if e.errorExpected && err == nil {
			t.Errorf("%s:error not expected, but none received", e.name)
		}

		if !e.errorExpected && err != nil {
			t.Errorf("%s:error expected, but one received: %s", e.name, err.Error())
		}
